...
class QuasarHotdealInfo:
    def __init__(self, num, category, title, price, link):
        self.num = num
...
def run_scraping(_webhook, _mode, _category, _keyword):
    logger.info('[퀘이사존 지름/할인정보 크롤링 시작]')
    
    # 페이지 소스 가져오기
    response = requests.get(quasar_url)
    if response.status_code == 200:
        ...
        # 스크래핑 시작
        div_result = soup.find('div', 'market-type-list')
        tr_result = div_result.find('tbody').find_all('tr')
        
        for i in range(len(tr_result)):
            cont_div = tr_result[i].find('div', 'market-info-list-cont')
            # 데이터 가져오기
            
            # 글번호
            temp_num = cont_div.find('a', 'subject-link').get('href').split('/')[4]
            
            # 카테고리
            temp_category = cont_div.find('span', 'category').get_text().replace('\n', '').strip()
            
            # 글제목
            temp_title = cont_div.find('a', 'subject-link').get_text().replace('\n', '').strip()
            
            # 가격
            temp_price = cont_div.find('span', 'text-orange').get_text().replace('\n', '').strip()
            
            # 링크
            temp_link = 'https://quasarzone.com/' + cont_div.find('a', 'subject-link').get('href')
            
            # DB 연결
            con = sqlite3.connect("./temp_list.db", isolation_level=None)
    
            # 게시글 중복 유무 확인
            select_count_query = "SELECT count (*) FROM quasar_list WHERE num=" + temp_num + ";"
            cursor = con.cursor()
            ...
            if select_count_result != 1:
                ...
                hotdeal_info = QuasarHotdealInfo(temp_num, temp_category, temp_title, temp_price, temp_link)
    
                insert_data_query = "INSERT INTO quasar_list (num, category, title, price, link) VALUES (?, ?, ?, ?, ?)"
                cursor = con.cursor()
                cursor.execute(insert_data_query, (int(temp_num), temp_category, temp_title, temp_price, temp_link))
                cursor.close()
                ...
                del hotdeal_info
                del webhook_conf
        logger.info('[퀘이사존 지름/할인정보 크롤링 종료]')
